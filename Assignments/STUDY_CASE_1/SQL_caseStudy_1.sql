...
Create function getFactProduct(
 @productt varchar(100)
)
returns table
as
return 
	select f.ProductId, p.Product_Type, f.Area_Code, f.Budget_Sales, f.Date from dbo.fact as f JOIN dbo.Product as p on p.ProductId = f.ProductId
	where Product_Type = @productt
GO

SELECT *
FROM getFactProduct('Coffee');
...
SELECT DENSE_RANK() over (ORDER by f.ProductId) as 'RANK',f.date as 'Date', p.ProductId, f.sales, f.Total_Expenses from dbo.Product as p JOIN dbo.fact as f 
ON p.ProductId = f.ProductId
WHERE f.Total_Expenses >100 and f.Total_Expenses <200
...
SELECT ASCII(SUBSTRING(Product, 5, 2)) AS asciiChar FROM dbo.Product;